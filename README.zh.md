...
如果你在用 IB API 做期权自动化交易，一定遇到过这些问题：

- **多个模块共享一个 TWS 连接**——如何避免 ClientID 冲突？
- **需要批量获取期权链数据**——如何高效率地拉取而不被 API 限速？
- **限价单总是无法成交**——如何自动调整价格来提高成交率？
- **波动行情中假信号频发**——如何防止风控被击穿？

**ibkr-options-cookbook** 用干净、模块化的参考代码来回答这些问题——以 SPX 铁鹰策略（Iron Condor）作为贯穿案例。它不是拿来就跑的交易机器人，而是一本经过实战检验的架构设计模式集。
...
**这是拿来就能跑的交易机器人吗？**
不是。这是架构设计模式的知识库和参考代码。你需要自己提供策略逻辑和风险参数。

**需要实盘 TWS 连接吗？**
需要。代码模块连接 TWS 或 IB Gateway。开发和测试阶段用模拟账号加行情订阅即可。

**示例中的铁鹰策略是什么？**
铁鹰策略是一种非方向性期权策略，同时卖出虚值看跌价差和看涨价差。标的在预设范围内波动时盈利。

**这些模块能用于其他策略吗？**
完全可以。连接管理、数据获取、订单生命周期、风控组件都是策略无关的，适用于任何期权或股票策略。
